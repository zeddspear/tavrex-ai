import { z } from 'zod';
import { segmentSchema } from './recording';
...
export const uploadMimeTypes = [
  ...
  'audio/mpeg', 'audio/wav', 'audio/x-wav', 'audio/mp4', 'audio/x-m4a', 'audio/webm',
] as const;
...
export const createUploadSchema = z.object({
  ...
  filename: z.string().trim().min(1).max(200)
    .refine((name) => !/[\\/]/.test(name) &&
      [...name].every((character) => character.charCodeAt(0) >= 32), 'Use a plain filename'),
...
export const storedTranscriptSchema = z.array(segmentSchema).max(5000).superRefine(
  (segments, context) => {
    const ids = new Set<string>();
    segments.forEach((segment, index) => {
      if (ids.has(segment.id) || segment.start >= segment.end ||
        (index > 0 && segment.start < segments[index - 1].end)) {
        context.addIssue({ code: 'custom', path: [index], message: 'Invalid transcript order or timing' });
      }
      ids.add(segment.id);
    });
  },
...
export function normalizeTranscription(value: unknown, duration: number) {
  if (!Number.isFinite(duration) || duration <= 0 || duration > uploadLimits.seconds)
  ...
  for (const item of response.segments) {
    ...
    segments.push({
      id: `segment-${segments.length + 1}`, speakerId: 'speaker',
      start, end, paragraphs: [text],
    });
  }
...
export function retryStage(hasUploadedMedia: boolean, hasStoredTranscript: boolean) {
...
export const processingMessages = {
  upload_failed: 'The upload did not finish. Choose the recording again to retry.',
  transcription_failed: 'We couldn’t transcribe this recording. Your uploaded media is saved; try again.',
  analysis_failed: 'Transcript complete. AI analysis failed. You can use the transcript or retry analysis.',
  processing_timeout: 'Processing took too long. Retry to continue from the last saved step.',
